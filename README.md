...
- 支持将多个扫描任务依次排队；
- 支持一键选择起止日期：自动扫描尚未完成的录像，再合成一条多日延时视频；
...
2. 选择摄像头与性能档位，填写可选的结束日期后点击“一键人物延时”。系统会先把范围内实际有录像、尚未完成的日期按时间顺序加入扫描队列，再自动合成一条成片。
3. 需要精细控制时，仍可分别使用“扫描范围人物”和“制作范围延时”。
...
第二个参数是本次最多处理的未处理 MP4 数量（例如 `5`），可随时按 `Ctrl-C` 停止；已完成文件会记录在 NAS 的 `processed.json`，下次运行自动续接。首次运行只会在仓库目录创建 `.mac-worker-venv` 独立虚拟环境，不修改系统 Python、Homebrew 或 Docker。运行 Mac worker 时，请不要让 NAS 对同一日期执行扫描任务。
