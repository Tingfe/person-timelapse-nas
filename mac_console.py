...
<main><div class=tag>LOCALHOST ONLY · MPS / METAL WORKER</div><h1>Mac 影像协处理</h1><div class=grid><section class=card><div class=tag>任务设定</div><label>录像日期<input id=date type=date required></label><label>执行批数 <input id=batches type=number min=1 max=50 value=1></label><div class=actions><button id=start>开始协处理</button><button class=stop id=stop>停止当前批次</button></div><p class=note>每批最多处理 5 个未处理文件。可随时停止；完成结果已写回 NAS，下一次会自动续接。</p></section><section class='card status'><div class=tag style='color:#a9caba'>运行状态</div><p><span id=dot class=dot></span><strong id=status>检查中</strong></p><p id=detail>仅监听 127.0.0.1，不对局域网开放。</p></section><section class=card style='grid-column:1/-1'><div class=tag style='margin-bottom:10px'>实时日志</div><pre id=logs>正在连接本地工作节点…</pre></section></div></main><script>
const $=s=>document.querySelector(s),api=(url,opt)=>fetch(url,opt).then(async r=>{const d=await r.json();if(!r.ok)throw Error(d.error);return d});async function refresh(){try{const s=await api('/api/status');$('#status').textContent=s.running?'正在协处理':'空闲';$('#detail').textContent=s.detail;$('#dot').className='dot '+(s.running?'run':'');$('#logs').textContent=s.logs.join('\\n')||'尚无日志';$('#logs').scrollTop=$('#logs').scrollHeight}catch(e){$('#status').textContent='本地服务异常';$('#detail').textContent=e.message}}$('#start').onclick=async()=>{try{const date=$('#date').value.replaceAll('-','');if(!date)throw Error('请选择录像日期');await api('/api/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date,batches:+$('#batches').value})});refresh()}catch(e){alert(e.message)}};$('#stop').onclick=()=>api('/api/stop',{method:'POST'}).then(refresh);refresh();setInterval(refresh,1500)</script></html>"""
...
def run_worker(date, batches):
    global PROCESS
    try:
        with LOCK:
            PROCESS = subprocess.Popen(["sh", str(ROOT / "mac-worker.sh"), date, str(batches)], cwd=ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            data=json.loads(self.rfile.read(int(self.headers.get('Content-Length','0'))));date=data['date'];batches=int(data.get('batches',1))
            ...
            threading.Thread(target=run_worker,args=(date,batches),daemon=True).start();return self.reply(status())
